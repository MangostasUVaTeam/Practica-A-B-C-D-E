...
Vamos a investigar como es el paso de una ruta (``PATH``)determinada a su corresponiente inodo. Sabemos que esto se hace a través de la función ``eat_path`` alojada en ``/usr/src/fs/path.c`` por lo tanto crearemos una función que llamamos ``print_path_info`` que hará será traducir una ruta como *string* dada a través del ``message`` y gracias a la función ``eat_path`` recibamos su inode y una vez hayamos obtenido su inode tan solo tendremos que llamar a la función construida anteriormente y que llamamos ``print_inode_info``.

Mientras haciamos esto nos ha surgido un problema, que es el cómo pasar el string del programa de usuario al sistema de ficheros. La forma en que lo hemos probado es gracias al  campo ``m3_ca1`` que es un array de carácteres y según el fichero ``/usr/src/fs/param.h`` podemos acceder a él con tan solo escribir ``pathname`` pero este atributo de ``message`` está limitado a muy pocos carácteres gracias a su definición y ``M3_STRING`` que tiene definido el valor **14**. Al enviarlo desde los campos de tipo ``(char *)`` no funciona ya que gracias al **MMU** los procesos tienen direcciones de memoria relativas, es decir, el puntero apunta a zonas distintas según el proceso que utilice dicha variable.

Tras darle varias vuelta hemos llegado a la conclusión de que hay dos soluciones posibles, una de ellas es aumentar el tamaño del array ``m3_ca1`` y la otra crear un ``mess_7``. Vamos a optar por la segunda de las opciones ya que en el caso de que aumentasemos el tamaño de ``m3_ca1`` esto afectaría a todas las llamadas que utilizasen este atributo probablemente reduciendo el rendimiento del sistema, ya que nuestra nueva variable tendrá una longitud de **255** carácteres, la longitud máxima que puede tener una ruta (path) en minix. Además otra de las ventajas de crear el ``mess_7`` es que como está dentro de un ``union`` en el caso de que no demos valor a esa variable no ocupará espacio de memoria. Esto es lo que diferencia un ``struct`` de un ``union``. Asi que una vez explicados los motivos que nos llevan a ello, comenzaremos a hacerlo.

Lo primero que hacemos es localizar la definición de ``message`` que está en ``/usr/include/minix/type.h`` y procedemos a crear el nuevo campo. Lo hacemos imitando la definición de los demás ``struct`` y ``m3ca1`` también definiendo la constante ``M7_STRING`` con el valor **255**. También definimos debajo ``m7_ca1`` para que sea más rapido acceder y legible acceder a este campo tal y como está echo con el resto.
